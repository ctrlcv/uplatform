...
class ExpertEditProfilePage extends StatefulWidget {
  const ExpertEditProfilePage({Key? key}) : super(key: key);

  static const routeName = '/ExpertEditProfilePage';

  @override
  _ExpertEditProfilePageState createState() => _ExpertEditProfilePageState();
}

class _ExpertEditProfilePageState extends State<ExpertEditProfilePage> {
  final _formKey = GlobalKey<FormState>(debugLabel: 'ExpertEditProfilePage');
  ...
  PartnerInfo? _loginUserPartnerInfo;
  ...
  void loadUserData() {
    ...
    _loginUserPartnerInfo = LoginService().getUserPartnerInfo();
    ...
    if (_loginUserPartnerInfo != null) {
      _selectedExpertType = _loginUserPartnerInfo?.partnerType ?? "";

      if (_selectedExpertType == "CS") {
        _selectedExpertType = "위생정리 전문가";
        _selectedCleanPositionTypes = _loginUserPartnerInfo?.position ?? "";
        _selectedCleanCareerCount = _loginUserPartnerInfo?.confirmHistory ?? "";
        _selectedCleanScopeOfActivity = _loginUserPartnerInfo?.activityDistance ?? "";
      } else if (_selectedExpertType == "CR") {
        _selectedExpertType = "공간정리 전문가";
        _selectedSpacePositionTypes = _loginUserPartnerInfo?.position ?? "";
        _selectedSpaceCareerCount = _loginUserPartnerInfo?.confirmHistory ?? "";
        _selectedSpaceScopeOfActivity = _loginUserPartnerInfo?.activityDistance ?? "";
      } else if (_selectedExpertType == "LC") {
        _selectedExpertType = "정리전문가 강사";
        _selectedEduCareerCount = _loginUserPartnerInfo?.confirmHistory ?? "";
        _selectedEduScopeOfActivity = _loginUserPartnerInfo?.activityDistance ?? "";
      }

      _isPersonalBusiness = ((_loginUserPartnerInfo?.bizType ?? "") == "개인사업자");
      _isCompanyBusiness = ((_loginUserPartnerInfo?.bizType ?? "") == "법인사업자");
      _isNoneBusiness = ((_loginUserPartnerInfo?.bizType ?? "") == "없음");

      _selectedWorkType = _loginUserPartnerInfo?.job ?? "";
      _businessNumberEditController.text = _loginUserPartnerInfo?.regNo ?? "";
      _businessNameEditController.text = _loginUserPartnerInfo?.bizName ?? "";
      _ceoEditController.text = _loginUserPartnerInfo?.ceoName ?? "";
      _addressEditController.text = _loginUserPartnerInfo?.address ?? "";
      _addAddressEditController.text = _loginUserPartnerInfo?.addAddress ?? "";
      _housePhoneEditController.text = _loginUserPartnerInfo?.tel ?? "";

      String licenseImages = _loginUserPartnerInfo?.licenseImages ?? "";
      if (licenseImages.isNotEmpty) {
        List<String> loadedImageFiles = (licenseImages.replaceAll("[", "").replaceAll("]", "").replaceAll(" ", "")).split(",");
        for (int i = 0; i < loadedImageFiles.length; i++) {
          if ((loadedImageFiles[i].trim()).isEmpty) {
            continue;
          }

          _selectCertificates.add(PickImageFile(fileType: PickImageFileType.networkFile, filePath: loadedImageFiles[i]));
        }
        debugPrint('init() _selectCertificates $_selectCertificates');
      }

      String regImages = _loginUserPartnerInfo?.regImages ?? "";
      if (regImages.isNotEmpty) {
        List<String> loadedImageFiles = (regImages.replaceAll("[", "").replaceAll("]", "").replaceAll(" ", "")).split(",");
        for (int i = 0; i < loadedImageFiles.length; i++) {
          if ((loadedImageFiles[i].trim()).isEmpty) {
            continue;
          }
          _selectBusinessRegistrations.add(PickImageFile(fileType: PickImageFileType.networkFile, filePath: loadedImageFiles[i]));
        }
        debugPrint('init() _selectBusinessRegistrations $_selectBusinessRegistrations');
      }
                  ...
                  child: BorderRoundedButton(
                    ...
                    onPressed: () async {
                      ...
                      if (uploadCertImages.isNotEmpty) {
                        ...
                        debugPrint("uploadCertificatesResult $uploadCertificatesResult");
                      ...
                      if (uploadRegImages.isNotEmpty) {
                        ...
                        debugPrint("uploadBusinessRegistrationsResult $uploadBusinessRegistrationsResult");
                      ...
                      CommonResponse result = await Network().reqUpdatePartner(params);
                      ...
                      if (result.status == "200") {
                        ...
                        if (_loginUserPartnerInfo != null) {
                          _loginUserPartnerInfo?.partnerType = _selectedExpertType;

                          if (_selectedExpertType == "위생정리 전문가") {
                            _loginUserPartnerInfo?.position = _selectedCleanPositionTypes;
                            _loginUserPartnerInfo?.confirmHistory = _selectedCleanCareerCount;
                            _loginUserPartnerInfo?.activityDistance = _selectedCleanScopeOfActivity;
                          } else if (_selectedExpertType == "공간정리 전문가") {
                            _loginUserPartnerInfo?.position = _selectedSpacePositionTypes;
                            _loginUserPartnerInfo?.confirmHistory = _selectedSpaceCareerCount;
                            _loginUserPartnerInfo?.activityDistance = _selectedSpaceScopeOfActivity;
                          } else if (_selectedExpertType == "정리전문가 강사") {
                            _loginUserPartnerInfo?.confirmHistory = _selectedEduCareerCount;
                            _loginUserPartnerInfo?.activityDistance = _selectedEduScopeOfActivity;
                          }

                          if (_isPersonalBusiness) {
                            _loginUserPartnerInfo?.bizType = "개인사업자";
                          } else if (_isCompanyBusiness) {
                            _loginUserPartnerInfo?.bizType = "법인사업자";
                          } else if (_isNoneBusiness) {
                            _loginUserPartnerInfo?.bizType = "없음";
                          }

                          _loginUserPartnerInfo?.job = _selectedWorkType;
                          _loginUserPartnerInfo?.regNo = _businessNumberEditController.text;
                          _loginUserPartnerInfo?.bizName = _businessNameEditController.text;

                          if (_selectCertificates.isNotEmpty) {
                            _loginUserPartnerInfo?.licenseImages = _selectCertificates.toString();
                          }

                          if (_selectBusinessRegistrations.isNotEmpty) {
                            _loginUserPartnerInfo?.regImages = _selectBusinessRegistrations.toString();
                          }
                          ...
                          _loginUserPartnerInfo?.address = address;

                          _loginUserPartnerInfo?.ceoName = _ceoEditController.text;
                          _loginUserPartnerInfo?.tel = _housePhoneEditController.text;
                        }
