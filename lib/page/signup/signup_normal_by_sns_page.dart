...
class SignUpNormalBySnsPage extends StatefulWidget {
  const SignUpNormalBySnsPage({Key? key}) : super(key: key);

  static const routeName = '/SignUpNormalBySnsPage';

  @override
  _SignUpNormalBySnsPageState createState() => _SignUpNormalBySnsPageState();
}

class _SignUpNormalBySnsPageState extends State<SignUpNormalBySnsPage> {
  final _formKey = GlobalKey<FormState>(debugLabel: 'SignUpNormalBySnsPage');
            ...
            child: Column(
              children: [
                  ...
                  child: Image.asset(
                    Get.arguments == "KAKAO"
                        ...
                        : Get.arguments == "NAVER"
                            ...
                            : "assets/images/icons_account_type_google.png",
                    width: 48,
                    height: 48,
                  ),
                ),
                ...
                if (_selectedJobTitle == "주부" ||
                  ...
                  buildHouseKeeperInputs(context)
                else if (_selectedJobTitle == "사장" || _selectedJobTitle == "직원")
                  buildRestaurantWorkerInput(context),
                Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 20),
                  child: BorderRoundedButton(
                    text: "회원가입",
                    ...
                    onPressed: () async {
                      ...
                      if (Get.arguments == "KAKAO") {
                        KakaoUser? kakaoUser = LoginService().getKakaoUser();
                        ...
                        NaverUser? naverUser = LoginService().getNaverUser();
                      ...
                      SignUpResponse result = await Network().reqRegisterUser(params);

                      if (result.status == "200") {
                        Map<String, dynamic> loginParams = {};
                        loginParams['sns_key'] = snsKey;
                        LoginUser user = await Network().reqSnsLogIn(loginParams);
                        ...
                        Get.toNamed(SignUpFinish.routeName);
                      } else {
  ...
  Widget buildRestaurantWorkerInput(BuildContext context) {
    return Column(
      children: [
        const InputTitle(
          title: "음식점 유형",
  ...
  Widget buildHouseKeeperInputs(BuildContext context) {
    return Column(
      children: [
        const InputTitle(
          title: "관심 서비스",
